...
项目目前处于早期开发阶段，里程碑 2 的通用键位与组合键模型已经可用；里程碑 3 的 i18n 基础正在收尾，当前非 GUI 范围已部分完成。
...
- 支持 `system`、`zh-CN` 和 `en-US` 界面语言；托盘菜单、错误摘要和配置语言项已接入 i18n，底层配置解析明细仍以英文日志和详情为主。
- 支持当前用户开机启动。
...
里程碑 3 当前状态：进行中，非 GUI 范围部分完成。后续开发将继续完成 i18n 风险收尾，再转向界面与发布准备：

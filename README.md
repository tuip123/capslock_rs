...
项目目前处于早期开发阶段，里程碑 2 的通用键位与组合键模型已经可用；里程碑 3 和里程碑 4 均为部分完成。当前已有原生 GUI 设置页、基础映射编辑器和映射行级状态，但保存前 INI 预览、发布准备等仍未完成。
...
- GUI 映射编辑器代码层面支持新增、更新、删除 `keyFunc_*`、`keyTarget_*`、`keyCombo_*` 三类映射，并在列表中显示正常、重复映射、非法输入组合、未知动作或配置错误状态；仍缺保存前 INI 预览和真实窗口完整回归。
...
映射编辑器代码层面已支持新增、更新和删除 CapsLock 输入组合，并配置三类目标：内置函数、目标单键和目标组合键。映射列表会复用配置解析和校验结果显示行级状态；保存时会先复用配置解析和校验规则，写入成功后自动重载运行时配置。

GUI 与手工编辑共用同一份 INI。已能识别的 `general`、`ui` 字段和三类 `[Keys]` 映射会在 GUI 保存后继续保留；但注释、未知键、原始顺序和原始格式目前不会保留。

当前仍未完成：

- 保存前尚未显示将写入的 INI 表达式预览。
...
- 补齐保存前 INI 预览，并继续打磨编辑期校验反馈。
- 继续验证中文和英文界面下的完整配置流程。
- 发布准备、许可证、CI 和发布包仍属于后续里程碑。
